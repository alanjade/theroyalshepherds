...
export function NewsFormDialog({ categories }: { categories: { id: string; name: string }[] }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);

  function close() { router.push("/admin/news"); }
  function handleSubmit(formData: FormData) {
    startTransition(async () => {
      const res = await createNews(formData);
      if (res.success) { router.push("/admin/news"); router.refresh(); } else setError(res.error);
    });
  }
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div className="bg-white rounded-xl2 shadow-card max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-royal-100">
          <h2 className="font-display font-bold text-royal-900">Publish News</h2>
        ...
        <form action={handleSubmit} className="p-5 space-y-4">
          ...
          <div><label className={labelClass}>Excerpt</label><textarea name="excerpt" rows={2} className={inputClass} /></div>
          <div><label className={labelClass}>Content (basic HTML supported, sanitized on save)</label><textarea name="content" rows={6} className={inputClass} /></div>
          <div>
          ...
          <label className="flex items-center gap-2 text-sm"><input type="checkbox" name="featured" className="rounded" /> Feature this article</label>
          <div>
